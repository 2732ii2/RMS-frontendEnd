...
return <div className="w-[100%] h-[100vh]   z-[20] relative   flex flex-col items-center ">
    <h1 style={{width: name.includes("dashboard") || name.includes("menu") ?"100%":""}} className={` ${name?.includes("dashboard") || name.includes("menu") ?"text-start pl-5 pt-2":"mt-[100px] "}  text-[32px] font-bold  tracking-wide     px-3    `}>  Saida Solutions</h1>
    { name.includes("dashboard") || name.includes("menu")?<div className="  absolute   w-[auto] h-[auto] top-[10px] right-[30px] ">
        <button onClick={()=>{
            ...
            // },500)
            dispatch(userSess({}));
        }} className="  font-semibold   px-2 py-1 rounded-[4px] bg-[rgba(0,0,0,.9)] transition-all text-white active:skew-y-3">Log OUT</button>
    </div>:null }
